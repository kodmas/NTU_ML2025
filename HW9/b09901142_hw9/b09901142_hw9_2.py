...
def sce(
    ...
    if density < 1.0:
        mask = sce_mask(task_tensors, density=density, mask_dtype=task_tensors.dtype)
    ...
    repeated_weights = weights * majority_sign_mask
    ## Weighted summation of masked task tensors to create a merged task tensor.
    mixed_task_tensors = (task_tensors * repeated_weights).sum(dim=0)
    ...
    num_params_preserved = torch.clamp(repeated_weights.sum(dim=0), min=1e-6)
    mixed_task_tensors = mixed_task_tensors / num_params_preserved
    return mixed_task_tensors
